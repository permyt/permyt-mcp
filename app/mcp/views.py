...
class OAuthAuthorizeView(View):
    ...
    def get(self, request):
        ...
        except (OAuthAuthorizationSession.DoesNotExist, ValidationError):
...
class OAuthCallbackView(View):
    ...
    def _authenticate(self, request):
        ...
        if login_id:
            ...
            except (LoginToken.DoesNotExist, ValidationError) as exc:
    ...
    def _load_oauth_session(self, request):
        ...
        except (OAuthAuthorizationSession.DoesNotExist, ValidationError) as exc:
    ...
    def get(self, request):
        try:
            ...
            logger.warning(f"OAuth callback failed: {exc}")
            return HttpResponseBadRequest(str(exc))

        user = request.user
        if not user.permyt_user_id:
            logger.warning(f"OAuth callback: user {user.id} has no permyt_user_id")
        ...
        logger.info(
            f"OAuth callback success: user={user.id} client={oauth_session.client_id[:12]} "
            f"redirect={redirect_uri[:80]}"
        )
